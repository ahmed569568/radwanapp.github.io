...
export class WhishlistCartComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    setTimeout(() => {
      if (String(this.router.url).includes("/whishlist-cart/cart")) {
        ...
        String(this.router.url).includes("/whishlist-cart/whishlist")
      ) {
        this.whishlistStyle.nativeElement.style.background =
          "rgba(231, 76, 60, 1)";
  ...
  activateTab(value: any) {
    if (
      String(this.router.url).includes("/whishlist-cart/cart") &&
      value != "cart"
    ) {
      ...
      if (value == "wishlist") this.router.navigate(["./whishlist-cart/cart"]);
      else value == "cart";
      this.router.navigate(["./whishlist-cart/whishlist"]);
    }
  }
  ...
  apply() {
    this.promotionService
      .apply(this.promotion.value, this.storage.get("cart"))
      .subscribe(
        (data: any) => {
